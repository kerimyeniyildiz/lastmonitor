...
def looks_like_luleburgaz_campaign_handle(handle: str) -> bool:
...
def evaluate_tweet_filter(config: Config, search_query: str, tweet: Dict) -> List[str]:
    ...
    if has_link and location_mentions >= 3 and meaningful_length <= 45:
        reasons.append("watch_pattern:location_word_soup_link")
    ...
    if (
        ...
        reasons.append("watch_pattern:suspicious_location_link")

    if is_luleburgaz_query(search_query) and has_link:
        handle = str(tweet.get("user_handle") or "")
        user_name = str(tweet.get("user_name") or "")
        short_campaign_handle = looks_like_luleburgaz_campaign_handle(handle)
        ...
        if (
            ...
            reasons.append("watch_pattern:luleburgaz_short_link_campaign")
        ...
        if (
            ...
            reasons.append("watch_pattern:luleburgaz_ad_profile")
        if (
            profile_campaign_handle
            and campaign_location_mentions >= 3
            and text.count(",") >= 5
        ):
            reasons.append("watch_pattern:luleburgaz_location_dump")
